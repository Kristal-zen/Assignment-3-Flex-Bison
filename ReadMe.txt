...
Description:
	This projects aim is to implement a statement parser whose job is o validate and evaluate statements based on set restrictions on grammar. The parser will accept the following: 
	      I. Assignments in the form of: id = expression;
	      II. Expressions in the form of: id op id { op id }
	Where each line in the input text file is treated as its own separate statement. the program will read statements from a file (scanme.txt for example) then parses them and will print one of the following:
	      I. valid
	      II. invalid: <reason>
	With the reason including information like: invalid tokens and malformed assignments or expressions. Overall this project will use flex for scanning/ tokenizing the input, bison for parsing and gcc builds the final executable .
...
File Manifest:
	...
	- parser.y                        # The bison grammar which defines valid expression rules and assignments
	...
	- main.o			  # Has a helping hand in the compilation of the program
	- ReadMe.txt			  # This document, outlines and explains the assignment/ program
...
Operating Instruction:
	Here are the steps to follow in order to use the program to it's full potential:
	  ...
	  2. Next you want to run this command: scanner <testfile.txt> with test file replaced with your file of choice.
	  3. From here you can see that the program wall print each of the statements followed by either a '-- valid' or  '-- invalid: <reason>'
...
Design Decisions:
  ...
  -Operators: For operators only the following are recognized: +, -, *, /, % with any other operators being considered an invalid token
  ...
  -Error Reporting: The first invalid token will be caught by the scanner, if there are no invalid tokens but the grammar still fails then that means that A. if an assignment was attempted = issue with invalid assignment, or B. otherwise, that means that it is an issue with an invalid expression.

Description of all known deficiencies or bugs:
	-Truncation Warning: There is a warning that will occur when you first compile the files, but this does not affect the output. What this warning is telling the user is that long invalid tokens may be shortened down when copied into error messages, not affecting the runtime of the program.
	-Minimal expression (Grammar): Although it is part of the assignment to have such a simple grammar set up but this is set up in a way that only supports: left to right sequences
	-Trailing Input: For some expression lines, anything that occurs after a valid expression (that is NOT whitespace) can cause a 'invalid expression' rather than a more specific error
